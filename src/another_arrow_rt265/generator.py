...
class LevelSpec:
    ...
    ``rows`` / ``cols`` 本身没有硬上限（只要 ≥ 1），但**建议不要超过 16x16**：
    默认 720x720 窗口下 16x16 的格子只剩 34px，再大箭头就看不清了；生成侧也只
    验证到 24x24。两个上限的实测数据见 :func:`generate_level` 的尺寸说明。
    """
...
def generate_level(
    ...
    Returns:
        ...
        棋盘越满越难摆：越到后面越难找到“前方没有箭头”的方向，所以能摆下的
        箭头数量有上限，而且**上限跟着棋盘尺寸一起降**——实测 6x6 能塞到 34 支
        （密度 ≈ 0.94）、12x12 约 101 支（≈ 0.70）、24x24 约 271 支（≈ 0.47），
        再往上成功率迅速掉到零。想要固定的箭头数量，请用
        :func:`generate_solvable_level`，它会自动重试。

    Note:
        尺寸上限是**实测**的，不是算法保证的，而且**显示侧比生成侧先失效**：默认
        720x720 窗口留给棋盘的区域是 656x552，格子边长 = ``552 // rows``，实测
        14x14（格子 39px）以内观感舒服、16x16（34px）仍可读、18x18（30px）笔画
        发糊、20x20（27px）以上四个方向分不出来。因此**建议棋盘不超过 16x16**，
        更大的棋盘不保证可用：生成侧的密度上限也会随尺寸一起降（见上，只验证到
        24x24）；棋盘虽然跟着窗口等比放大，但得把窗口一起放大才顶得住
        （1080x1080 下 24x24 的格子才回到 34px）。
    """
