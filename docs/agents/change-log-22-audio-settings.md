...
> **更正（2026-09-22，见 `change-log-24-ui-copy-policy.md`）**：本文第 1 节与第 5 节所说的
> “设置页有说明卡片（452~580）与页脚提示行（588~612）”**与代码不符**——`settings_page()`
> 从来没有 `sections` / `hint`，这是**用户有意删除**的文案。现行规则是“界面不写多余的
> 说明文字”（`AGENTS.md` 的 UI Copy Notice），相关测试已改。下文保留原样只作历史记录。
